...
# 一键安装 + 美化 vnStat Telegram 流量统计（彩色进度条 + 彩色箭头）
set -euo pipefail
IFS=$'\n\t'
...
📊 进度: $BAR $PCT%   ⚡ 流量状态: $STATUS"
    curl -s -X POST "$TG_API_BASE" -d chat_id="$CHAT_ID" --data-urlencode "parse_mode=Markdown" --data-urlencode "text=$MSG" >/dev/null 2>&1
}
